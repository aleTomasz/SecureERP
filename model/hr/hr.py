...
def average_age():
    ...
    for i in list:
        sum_age += i[0]
    return 2024 - int(sum_age / len(list))
...
def get_next_birthdays(input_date):
    ...
    for i in range(len(day_counter_list)):
        ...
        if (requested_day_counter_list[0][-1] - abs(year_difference)) % day_counter_list[i][
            -1] == 0 and day_difference >= -14 and day_difference <= 0:
            birthday_names.append(file[i][NAME])
        if ((requested_day_counter_list[0][-1] - abs(year_difference))) % day_counter_list[i][
            -1] == 0 and day_difference >= 351:
            birthday_names.append(file[i][NAME])
